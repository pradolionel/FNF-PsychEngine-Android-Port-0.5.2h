	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
				...
				for (i in 0...controlArray.length)
				{
					if(controlArray[i])
					{
						switch(i)
						{
							...
							case 7:
								ClientPrefs.comboOffset[3] -= addNum;
					case 8:								ClientPrefs.comboOffset[4] -= addNum;							case 9:								ClientPrefs.comboOffset[4] += addNum;							case 10:								ClientPrefs.comboOffset[5] += addNum;							case 11:								ClientPrefs.comboOffset[5] -= addNum;
						}
					}
				}
				repositionCombo();
			...
			if (FlxG.mouse.justPressed)
			{
				...
				if (startMousePos.x - comboNums.x >= 0 && startMousePos.x - comboNums.x <= combo.width &&
				...
				else if (startMousePos.x - comboNums.x >= 0 && startMousePos.x - comboNums.x <= comboNums.width &&				{					holdingObjectType = 'numscore';					startComboOffset.x = ClientPrefs.comboOffset[2];					startComboOffset.y = ClientPrefs.comboOffset[3];					//trace('yo bro');
			}
